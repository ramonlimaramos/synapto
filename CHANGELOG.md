...
- **domain scopes are canonicalized in one place** (`synapto.domain_scope`): domains are trimmed and lowercased on write and on read, so `" Python "`, `"PYTHON"`, and `"python"` become one storage key instead of three mutually unreachable ones. Canonicalization runs inside `MemoryRepository.create` and the search filter builder rather than only at the MCP boundary, so CLI import, migrations, and future callers cannot reintroduce a write/read mismatch. Length is validated after normalization, so a padded 50-character domain is accepted instead of rejected. `synapto import` and `synapto search --domain` now report invalid domains as clean CLI errors — naming the offending payload item — instead of surfacing a raw `VARCHAR(50)` psycopg error.
