...
- Handoffs are one packet per task. The `agent_handoff` prompt and `agent_handoff_template` tool now instruct the writer to look the task's packet up first — `recall(task_id, metadata_filter={"kind": "handoff", "task_id": …})`, also under the legacy kind — and to extend it with `update_memory(append=…, metadata_patch={"status": …})`; `remember` is the path only when no packet exists. The earlier rule, "append a new handoff instead of mutating the original", had turned single tasks into four near-identical memories, each a stale snapshot ranking against the others. The create path prescribes the writing contract the packet was missing: `subtype: "handoff"`, `origin: "agent"`, `scopes: ["area:<discipline>"]` (a new `area` parameter, default `software-engineering`; the repository stays in `metadata.repo` because scopes gate reads), tenant omitted or `<owner>/workspace`. `metadata.kind` is `handoff` (schema version 2); readers accept `agent_handoff` for one release. `handoff_inbox` and `handoff_inbox_template` look packets up by `metadata_filter` on `kind`, `to_agent`, `status` and optionally `task_id` instead of ranking a keyword query, and end by extending the same packet (#117)
...
||||||| parent of 6ebf320 (feat(handoff): one packet per task; templates prescribe the writing contract; update_memory accepts depth_layer)
...
- The `remember` tool description documents `scopes` — the seven types, an `area:` example, and the consequence that a memory carrying an `area` is returned only by scoped recalls that name it — and marks `domain` as the deprecated predecessor. `recall` already did; the writer never had, and the store showed it: three scope rows in 1,293 memories (#114)
...
- `tests/eval`: a golden-set retrieval harness. A synthetic ~200-memory corpus under tenant `acme/eval` (`corpus.toml`) and 55 cases across seven signals (`golden/<signal>.toml`: layer, trust, decay, hrr, scopes, metadata, general) are seeded through `remember` and run through `hybrid_search`; MRR@10 and Recall@5 per signal are compared with the committed `baseline.json`, and a move beyond ±0.02 in either direction fails, so a ranking change cannot merge without re-baselining (`SYNAPTO_EVAL_WRITE_BASELINE=1`) in the same PR. Runs inside the existing test job with the deterministic offline embeddings; no new dependency. The README's "Retrieval eval" section covers adding a case. The first baseline records the ranker as it is: the HRR boost's floor (≈0.075 at zero similarity) is larger than the largest possible RRF sum (≈0.033), so `(rrf + hrr) × weight` is decided by `quality_weight` rather than relevance inside the candidate window — removing the boost locally lifts overall MRR@10 from 0.51 to 0.90. That is input to #94, not something this change alters (#88)
- `tests/eval/ablation.py`: a ranking-signal ablation over the golden set, measurement only. Each signal — HRR boost, decay, trust, layer weight, vector leg, keyword leg — is switched off in turn as a variant of the production `RRF_QUERY_TEMPLATE` (exact fragment replacement that refuses to run if the statement drifted) plus a zeroed boost, applied inside the test process; a second block removes the boost first and then one more signal, because the boost's floor makes every other verdict under `full` an artifact of it. The noise floor is the spread of overall MRR@10 over five reseeds in shuffled insertion order, one per reference configuration. `SYNAPTO_EVAL_ABLATION=1 uv run pytest tests/eval` writes `docs/eval/ablation.md` with the run date, corpus digest and provider; `docs/eval/README.md` carries the reading. First run (2026-09-06): the HRR boost hurts retrieval by +0.38 MRR@10 (#103) and the layer weight's 3× spread costs +0.02 even without the boost (#104); trust and the keyword leg earn their place in both blocks; decay and the vector leg earn theirs once the boost is gone. Seeding and querying moved from `test_golden_set.py` into `tests/eval/seeding.py` so both the gate and the ablation use the same path; no production code changed (#94)

- `tests/unit/test_destructive_surface.py`: the destructive surface is pinned. A test walks the `synapto.sql` package by AST and lists every constant that deletes, soft-deletes, drops, truncates or moves rows between tenants; each one must be declared in a snapshot with its reach — MCP tool, CLI command or library-only — and the test modules that prove refusal, happy path and "the rest was not touched". A new destructive statement that is not declared fails the suite with a message naming the constant and what to declare; a stale declaration fails too, and every `-- migrate:down` section is checked for its rollback coverage. The same file fills the gaps the inventory exposed: `forget` leaves the sibling memory, the other tenant and the deleted row's scopes and entity links alone; the `maintain` tool — previously untested — soft-deletes only `ephemeral` memories older than `decay_ephemeral_max_age_hours`, never `working`/`stable`/`core` however old, and does not count an already-deleted row twice; `update_memory` rescopes or relinks only its own row and a rejected scope key changes nothing; the HRR bank rebuild deletes only the bank left with no vectors (#90)
- `tests/unit/test_maintain_merge_tenants_cli.py`: the first end-to-end test of `synapto maintain --merge-tenants --apply`, through `CliRunner` against a real `PostgresClient` on the disposable test database. Without a flag nothing moves and no alias is recorded; with `--apply` the memories of a name variant fold into the most populated spelling with their scopes, the alias is recorded and resolves, an unqualified spelling follows the single qualified one, the moved count equals what the dry run promised, groups claimed by two owners are held and do not block the actionable ones, a second apply finds nothing left, and tenants outside the plan keep their counts and the total is conserved. The first run of this test found the merge defect fixed below (#90)

### Changed

- `hybrid_search` breaks ties deterministically: the two candidate legs and the outer query order by `created_at DESC, id` after their score, so identical-content twins and templated notes with equal term overlap no longer come back in a different order — or a different top-20 — from one call to the next. Among equals the newest memory wins (#88)
...
- README caught up with 0.6.0 and 0.7.0: a "Tenants and scopes" section documents the tenant resolution order, the canonical `owner/name` form, `synapto maintain --merge-tenants`, typed scopes and how a scoped `recall` matches; the MCP tool table lists `agent_handoff_template`, `handoff_inbox_template` and the actual `list_entities_tool` name and the new `remember`/`recall`/`forget` parameters (`scopes`, `origin`, `metadata_filter`, `allow_human`); the field-limit table covers `domain`, `scopes`, `origin` and `metadata_filter`; the Claude Code setup uses `claude mcp add --scope user` and names `~/.claude.json` as the file Claude Code reads, with the `uv run --project` versus `--directory` working-directory caveat; `SYNAPTO_DEFAULT_TENANT` is described as a fallback; the CLI and Development sections list `init`, `migrate up/down`, `maintain`, `migrate-memories`, `configure-mcp`, the `uv` workflow and the two test-enforced conventions; examples no longer name real organisations

### Fixed

- `synapto maintain --merge-tenants --apply` promised in its dry run a merge it could not perform. A spelling that differs from its sibling only by case or by an underscore in the owner segment (`ACME-Manual/web` beside `acme-manual/web`) was planned as `[merge]`, but `TenantAliasRepository.merge` validated it as a canonical alias, so `--apply` died with an `InvalidTenantError` traceback after printing the plan — and after any earlier group in the same run had already been merged. The standard is now stated and enforced end to end: every tenant is lowercase. The planner only ever proposes a canonical target — the busiest member that already satisfies the grammar, else the lowercase fold itself — so a store holding only `Acme/API` is offered `acme/api` instead of being reported as unchanged while its memories stay unreachable; owners are compared folded, so `Acme/api` beside `acme/api` and a bare `api` is one owner's project (`[check]`) rather than a two-owner dispute or a group "with no 'owner/name' spelling"; `merge` accepts any stored spelling as the thing being folded away and records the alias only when that spelling is itself canonical, because a spelling the boundary rejects can never be looked up; and the whole plan runs as one transaction through the new `TenantAliasRepository.merge_all`, so a group refused midway undoes the ones before it and the command reports `nothing was applied — …` with exit 1 instead of a traceback (#90)
- `synapto configure-mcp --client claude-code` wrote `~/.claude/.mcp.json`, a path Claude Code never reads, and reported success; the running configuration stayed as it was. Claude Code resolves user-scoped servers from `~/.claude.json`, so the command now detects Claude Code by that file or the `~/.claude` directory and rewrites only `mcpServers.synapto` there — every other key in the file, which holds unrelated client state, is carried over unchanged and the write goes through a sibling temporary file so a failure cannot truncate it. An entry left in the legacy `~/.claude/.mcp.json` seeds the new one when `~/.claude.json` has none, and the command names that file as unread. A file that is not a JSON object is refused with the path and position, untouched. The entry that will load is printed after each write. `synapto init` reaches the same detection. `docs/claude-code.md` no longer points at `~/.claude/.mcp.json` or `.claude/settings.json` (#99)
- The depth-layer weights in `hybrid_search` were `1.5 / 1.2 / 1.0 / 0.5` for `core / stable / working / ephemeral`, a 3× spread chosen by feel; the ablation for #94 measured it as the one signal that still cost retrieval once the HRR boost was gone, and the golden set's `layer` slice stood at 0.9167 with one `working` twin outranking its `stable` target. They are now `1.3 / 1.25 / 1.0 / 0.7`, chosen by a sweep of ten spreads over the golden set under `full` and `no-hrr` (`tests/eval/layer_sweep.py`; `SYNAPTO_EVAL_LAYER_SWEEP=1 uv run pytest tests/eval` writes `docs/eval/layer_sweep.md`). The rule: a spread is admissible when every `layer` case ranks its target first; the best overall MRR@10 among admissible spreads wins; a tie within the noise floor goes to the narrower. Only the chosen spread is both admissible and best — overall MRR@10 0.9208 → 0.9482, Recall@5 0.9818 → 1.0, `layer` 0.9167 → 1.0, `general` 0.8333 → 0.8861, no slice worse — and the regenerated ablation reads "earns its place" for the layer weight in both blocks. Narrowing alone did not work: a `working` memory that restates the query verbatim earns a full HRR leg, so `stable` needs about 1.25× over `working` to stay first, and the binding ratio is `stable / working` rather than the overall width. The two `CASE` arms in `synapto.sql.search` and `DEPTH_BOOST` changed together; `test_ranking_weights.py` is unchanged and still passes (#104)
- The HRR boost in `hybrid_search` was noise on the wrong scale, and the ablation for #94 measured it as the single most harmful ranking signal (+0.38 MRR@10 for removing it). Two defects: it added `((sim + 1) / 2) × 0.15` to the RRF sum, so an unrelated memory gained +0.075 against a largest possible RRF sum of ≈0.033 and the quality weight decided the order inside the candidate window; and it compared an unbound `encode_text(query)` with a memory stored as `encode_fact(content, entities)`, a role-bound vector, so the similarity carried no signal — no golden case ranked its target first on HRR alone. HRR is now a leg on the RRF scale: the query is encoded as `remember` encodes the memory, similarity within the noise floor (`similarity_noise_floor`, 3σ of the phase cosine at the vector's dimension, ≈0.066 at 1024) counts as no match, and a candidate gains `evidence / (rrf_k + 1)` — nothing when unrelated, exactly one leg for an identical vector. A third reciprocal-rank leg, the option #103 recommended, was measured first and rejected: 0.8771 overall MRR@10, below the 0.8965 of no HRR at all, because a twenty-row window ranks densely and hands a full leg to weak evidence. Golden set re-baselined 0.5138 → 0.9208 overall (`hrr` 0.25 → 0.86, `general` 0.18 → 0.83, Recall@5 0.78 → 0.98); ablation regenerated, every signal now earns its place under `full`. The probe is encoded once per search, not once per candidate (#103)
